...
function onEndSong()
    if isStoryMode == true and cutsceneFinished == false then
        if getModSetting('cutscenes') ~= 'Disabled' then
...
function onTimerCompleted(tag, loops, loopsLeft)
    ...
    if tag == 'setUpCutscene' then
        startTween('moveCamera', 'camGame.scroll', {x = 1539 - screenWidth / 2, y = 833.5 - screenHeight / 2}, 2, {ease = 'quadInOut'})
        doTweenZoom('zoomOutCamera', 'camGame', 0.69, 2, 'quadInOut')
    end
